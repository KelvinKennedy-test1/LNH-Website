...
// Close the mobile menu whenever any nav link is used to leave the page
...
if (document.getElementById('careersGrid')) {
  (function () {
    ...
    /* ── Load opportunities (falls back to defaults) ── */
    function loadOpportunities() {
      try {
        const stored = localStorage.getItem('lnh_careers');
        if (stored) {
          const parsed = JSON.parse(stored);
          if (Array.isArray(parsed) && parsed.length > 0) {
            return parsed.filter(o => o.published !== false);
          }
        }
